...
**Symptom:** when the user runs `/wiki-digest`, the skill body executes in the parent session instead of forking into a fresh context. This pollutes the parent context with the inbox + wiki tree + Rules.md but otherwise still works.

## Procedure

1. The wiki-digest skill body's primary path is `context: fork` + `agent: wiki-curator`. If forking is unavailable, the skill body still runs — the current Claude in the parent session reads the body and executes it.
...
2. To preserve the curator's behavior without true context isolation, the parent Claude must:
   ...
   - Continue using the wiki-digest skill body's lifecycle steps (archive-before-write, post-write audit).
...
The wiki-digest skill body can detect the fallback path by checking the environment variable `CLAUDE_CODE_FORK_SUBAGENT`:

```bash
if [ "${CLAUDE_CODE_FORK_SUBAGENT:-0}" != "1" ]; then
  echo "WARNING: forking unavailable; using fallback path. See .claude/skills/wiki-digest/reference/non-fork-fallback.md"
fi
```
