...
**Domain:** Claude Code skill + hook scaffolding that auto-maintains an Obsidian-compatible markdown wiki via an LLM session inbox
**Researched:** 2026-04-28
**Overall confidence:** HIGH

The stack is unusual in that there is essentially **no runtime to choose** — Claude Code itself is the runtime. The "stack" is a small set of file formats (skill markdown, sub-agent markdown, settings JSON) plus a thin shell script that backs the Stop hook. Decisions are about *which Claude Code primitive maps to which responsibility*, not which library to install.

Key locked choices, with rationale below:
...
These are common picks for "markdown automation" projects that **do not belong here**:
...
```
.claude/
├── settings.json                        # Stop hook registration only
├── hooks/
│   └── inbox-upkeep.sh                  # Reads stdin JSON, emits additionalContext JSON
├── skills/
│   ├── inbox-upkeep/
│   │   └── SKILL.md                     # In-session: how to mutate wiki/inbox/<file>.md
│   ├── digest-wiki/
│   │   ├── SKILL.md                     # context: fork, agent: digest-router
│   │   └── reference/
│   │       └── routing-examples.md      # Optional: example inbox→filed-note routings
│   └── wiki-rules/
│       └── SKILL.md                     # user-invocable: false; preloaded into digest-router
└── agents/
    └── digest-router.md                 # tools: Read, Write, Glob, Grep; skills: [wiki-rules]
```
...
These are illustrative, not finalized; they document the *shape* the roadmap must produce.
    ...
    "Stop": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "\"$CLAUDE_PROJECT_DIR\"/.claude/hooks/inbox-upkeep.sh",
            "timeout": 10
          }
        ]
      }
...
cat <<'EOF'
{
  "hookSpecificOutput": {
    "hookEventName": "Stop",
    "additionalContext": "Before responding to the next user message, run the /inbox-upkeep skill to refresh wiki/inbox/<session>.md as a state-of-the-world mirror of what's been built or decided this turn. Add, update, or prune atomic entries; do not append a chronological log."
  }
}
EOF
```
...
---
name: inbox-upkeep
description: Refresh the rolling session inbox at wiki/inbox/<session>.md as a state-of-the-world mirror of what has been built or decided. Use when the Stop hook nudges, or when the user asks to update the inbox.
...
---
name: digest-wiki
description: Convert the current session inbox into filed wiki notes per wiki/Rules.md. Run manually as a review checkpoint.
disable-model-invocation: true
context: fork
agent: digest-router
---

Process every entry in wiki/inbox/. For each:
1. Decide single-note vs split (1,000-word rule)
2. Choose category folder per Rules.md §2
3. Fill the _templates/note.md schema
4. Write the file under the chosen folder
5. Delete the entry from inbox/
6. Add cross-links via [[Title]] wiki-links

Return a summary of files created, files deleted, and any routing ambiguities.
```
...
---
name: digest-router
description: Routes inbox entries into filed wiki notes per Rules.md. Read-write, scoped to wiki/.
...
You are the wiki digest router. You receive the digest task as your prompt
(forked from the digest-wiki skill). Use the preloaded wiki-rules skill as
the authoritative contract. Operate only inside wiki/.
```
...
---
name: wiki-rules
description: Canonical wiki conventions (mirrors wiki/Rules.md). Preloaded into the digest-router sub-agent.
...
**If the consumer repo has no existing `.claude/` directory:**
...
**If the consumer repo already has `.claude/settings.json`:**
...
**If the consumer repo runs in plan mode often:**
...
**If the wiki lives outside the repo (e.g., a separate Obsidian vault):**
...
---
*Stack research for: Claude Code skill+hook scaffolding for an Obsidian-style codebase wiki*
*Researched: 2026-04-28*
